...
router.get('/ads', verifyToken, (req, res) => {
    ...
    db.query(sqlGetAds, [userId], (err, results) => {
        ...
        res.json({
            message: 'Объявления успешно получены', ads
        });
    });
});
...
router.get('/ads/:id', verifyToken, (req, res) => {
    ...
    db.query(sqlGetAdById, [adId, userId], (err, results) => {
        ...
        res.json({
            message: 'Объявление успешно получено', ad
        });
    });
});
// Создание объявления
router.post('/ads', verifyToken, (req, res) => {
    ...
    const sqlInsertAd = `
        INSERT INTO musician_search_ads (user_id, instrument_id, description, city_id, exp, exp_action, self_instr,
                                         exp_band, exp_band_action, base, self_creation, com_project, cover_band)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    db.query(sqlInsertAd, [userId, instrument_id, description, city_id, exp, exp_action, self_instr, exp_band, exp_band_action, base, self_creation, com_project, cover_band], (err, result) => {
        ...
        if (genre_ids && genre_ids.length > 0) {
            ...
            db.query(sqlInsertGenres, [genreValues], (err, result) => {
                ...
                res.json({
                    message: 'Объявление успешно создано', adId: adId
                });
            });
        } else {
            res.json({
...
router.put('/ads/:id', verifyToken, (req, res) => {
    ...
    const sqlCheckOwnership = `SELECT *
                               FROM musician_search_ads
                               WHERE id = ?
                                 AND user_id = ?`;
    db.query(sqlCheckOwnership, [adId, userId], (err, results) => {
        ...
        db.query(sqlUpdateAd, [instrument_id, description, city_id, exp, exp_action, self_instr, exp_band, exp_band_action, base, self_creation, com_project, cover_band, applicant_name, applicant_phone, adId, userId], (err, result) => {
            if (err) {
            ...
            const sqlDeleteGenres = `DELETE
                                     FROM musician_search_ad_genres
                                     WHERE ad_id = ?`;
